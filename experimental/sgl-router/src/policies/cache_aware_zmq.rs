...
impl CacheAwareZmqPolicy {
    ...
    /// looks for a *shallower* prefix holder that is still unqueued.
    fn best_overlap_spill_candidate(
        ...
        sample
            ...
            .max_by(|(ai, ao), (bi, bo)| {
                ao.cmp(bo)
                    .then_with(|| loads.load_of(&workers[*bi]).cmp(&loads.load_of(&workers[*ai])))
                    .then_with(|| bi.cmp(ai))
            })
...
mod tests {
    ...
    fn bimodal_selected_overlap_stays_within_the_matched_ceiling() {
        ...
        let chosen = policy.select(&workers, &ctx).expect("must pick");
        assert_eq!(chosen.url, "http://dspark:30000");
    ...
    fn bimodal_credits_a_worker_warm_under_the_other_hashing_family() {
        ...
        let tree = Arc::new(HashTree::new());
        // EAGLE owns the full bigram chain; DSPARK owns only the first two
