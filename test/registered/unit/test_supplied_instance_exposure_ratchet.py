...
# host, and `is_cuda()` / capability gates only open on real hardware -- a
# resolution write confined to a GPU handler would never enter the CPU
# registration's written set, leaving its supplied-instance readers unpinned.
# (No AMD registration: this test pins an *exact* set, and an `is_hip()`-gated
# write would shift it in ways none of the pinning hosts can verify; the ROCm
